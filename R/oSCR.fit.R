oSCR.fit <-
  function (scrFrame, model = list(D ~ 1, p0 ~ 1, sig ~ 1, asu ~ 1),
            ...
            hessian = T, print.level = 0, getStarts = FALSE) 
  {
    ...
    for (s in 1:ns) {
      if (!is.null(trimS)) {
        ...
        pixels.post <- apply(e2dist(scrFrame$traps[[s]][, 
                             c("X", "Y")], ssDF[[s]][, c("X", "Y")]), 2, min) <= trimS
    ...
    if (DorN == "N") {
      if (length(dens.fx) > 0) {
        ...
        if (!("session" %in% all.vars(model[[1]])) & !("Session" %in% all.vars(model[[1]]))) {
          names.n0 <- paste0("n0.")
          pars.n0 <- log(mean(unlist(lapply(scrFrame$caphist, 
                                            nrow))))
        }
      ...
      for (i in 1:nrow(Ys)) {
        if (is.null(trimS)) {
          ...
          if (i < nrow(Ys)) {
            ...
            trimR[[s]][[i]] <- apply(rbind(rep(trimS + 2, nrow(scrFrame$traps[[s]])), 
                                           e2dist(matrix(unlist(scrFrame$traps[[s]][pp, c("X", "Y")]), 
                                                         sum(pp), 2), scrFrame$traps[[s]][, c("X", "Y")])), 2, min) <= (2 * trimS)
            
            trimC[[s]][[i]] <- apply(rbind(rep(trimS + 2, nG[s]), 
                                           e2dist(matrix(unlist(scrFrame$traps[[s]][pp, c("X", "Y")]),
                                                         sum(pp), 2), ssDF[[s]][, c("X","Y")])), 2, min, na.rm = T) <= trimS
          }
          else {
            pp <- rep(T, ncol(Ys))
            trimC[[s]][[i]] <- apply(rbind(rep(trimS + 2, nG[s]), 
      ...
      for (s in 1:length(YY)) {
        ...
        if (distmet == "ecol") {
          ...
          D[[s]] <- costDistance(trLayer, as.matrix(scrFrame$traps[[s]][, 
                                 c("X", "Y")]), as.matrix(ssDF[[s]][, c(c("X","Y"))]))
        }
        if (smallslow) {
          if (distmet == "euc") {
            D <- list()
        ...
        for (i in 1:nrow(Ys)) {
          if (plotit) {
            ...
            points(ssDF[[s]][trimC[[s]][[i]], ], pch = 16, 
                   col = 2, cex = mycex)
            points(scrFrame$traps[[s]][trimR[[s]][[i]], c("X", "Y")], 
                   pch = 3, col = 4, cex = mycex, lwd = mycex)
          ...
          for (k in 1:nK[s]) {
            ...
            if (encmod == "B") 
              probcap <- c(plogis(a0[trimR[[s]][[i]]])) * 
              Kern[trimR[[s]][[i]], trimC[[s]][[i]]]
            if (encmod == "P") 
              probcap <- c(exp(a0[trimR[[s]][[i]]])) * 
              Kern[trimR[[s]][[i]], trimC[[s]][[i]]]
            if (!multicatch) {
              if (encmod == "B") {
                probcap[1:length(probcap)] <- c(dbinom(rep(Ys[i, 
                                                              trimR[[s]][[i]], k], sum(trimC[[s]][[i]])), 
              ...
              vvv <- rep(c(Ys[i, trimR[[s]][[i]], k], 1 - 
                             any(Ys[i, trimR[[s]][[i]], k] > 0)), sum(trimC[[s]][[i]]))
              vvv[vvv == 1] <- log(probcap[1:length(Pm)][vvv == 
                                                           1])
          ...
          lik.cond[trimC[[s]][[i]]] <- exp(colSums(Pm, 
                                                   na.rm = T))
        ...
        if(predict){
          ## need to save the likelihood bits here for each session
    ...
    msLL.sex <- function(pv, pn, YY, D, Y, nG, nK, hiK, dm.den, 
      ...
      for (s in 1:length(YY)) {
        ...
        for (i in 1:nrow(Ys)) {
          if (plotit) {
            ...
            points(ssDF[[s]][trimC[[s]][[i]], ], pch = 16, 
                   col = 2, cex = mycex)
            points(scrFrame$traps[[s]][trimR[[s]][[i]], 
          ...
          if (multicatch) 
            Pm <- Pm1 <- Pm2 <- matrix(0, sum(trimR[[s]][[i]]) + 
                                         1, sum(trimC[[s]][[i]]))
          if (!multicatch) 
            Pm <- Pm1 <- Pm2 <- tmpPm <- matrix(0, sum(trimR[[s]][[i]]), 
                                                sum(trimC[[s]][[i]]))
          if (!is.na(sx[i])) {
            for (k in 1:nK[s]) {
              if (pBehave) {
              ...
              if (encmod == "B") 
                probcap <- c(plogis(a0[trimR[[s]][[i]]])) * 
                exp(-alphsig[s, sx[i]] * D[[s]][trimR[[s]][[i]], 
                                                trimC[[s]][[i]]]^2)
              if (encmod == "P") 
                probcap <- c(exp(a0[trimR[[s]][[i]]])) * 
                exp(-alphsig[s, sx[i]] * D[[s]][trimR[[s]][[i]], 
            ...
            for (k in 1:nK[s]) {
              ...
              if (encmod == "B") 
                probcap <- c(plogis(a0.1[trimR[[s]][[i]]])) * 
                exp(-alphsig[s, 1] * D[[s]][trimR[[s]][[i]], 
                                            trimC[[s]][[i]]]^2)
              if (encmod == "P") 
                probcap <- c(exp(a0.1[trimR[[s]][[i]]])) * 
                exp(-alphsig[s, 1] * D[[s]][trimR[[s]][[i]], 
                                            trimC[[s]][[i]]]^2)
              if (!multicatch) {
                if (encmod == "B") {
              ...
              if (encmod == "B") 
                probcap <- c(plogis(a0.2[trimR[[s]][[i]]])) * 
                exp(-alphsig[s, 2] * D[[s]][trimR[[s]][[i]], 
                                            trimC[[s]][[i]]]^2)
              if (encmod == "P") 
                probcap <- c(exp(a0.2[trimR[[s]][[i]]])) * 
                exp(-alphsig[s, 2] * D[[s]][trimR[[s]][[i]], 
                                            trimC[[s]][[i]]]^2)
              if (!multicatch) {
                if (encmod == "B") {
            ...
            lik.cond1[trimC[[s]][[i]]] <- exp(colSums(Pm1, 
                                                      na.rm = T))
            lik.cond2[trimC[[s]][[i]]] <- exp(colSums(Pm2, 
                                                      na.rm = T))
      ...
      if (predict) {
        return(list(preds = preds, ss.bits=ss.bits, lik.bits=lik.bits, ssDF=ssDF,
                    data = YY, traps = scrFrame$traps))
      }
    }
    if (getStarts == TRUE) {
      ...
      if (!predict) {
        ...
        output <- list(call = cl, rawOutput = myfit, outStats = outStats, 
                       ...
                       Area = areaS, ED = ED, nObs = unlist(lapply(scrFrame$caphist,nrow)),
                       mmdm = mmdm, nll = myfit$minimum, AIC = 2 * myfit$minimum + 2 * length(myfit$estimate), 
