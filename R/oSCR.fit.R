oSCR.fit<- function (model = list(D ~ 1, p0 ~ 1, sig ~ 1, asu ~ 1), scrFrame, 
    ...
    nlmiterlim = 200, smallslow = FALSE, print.level = 0) 
{
    ...
    pars.sig[1] <- log(0.5 * mean( mmdm ) )  # andy did this
    if (anySex) {
        if (sexmod == "constant") {
            pars.sex <- 0
    ...
    msLL.nosex <- function(pv = pv, pn = pn, D = D, hiK = hiK, 
  ...
  cat("nv: ", nv[1:nind], fill=TRUE)
  cat("lik.marg: ", lik.marg[1:nind],fill=TRUE)
                }
                if (telem) {
    ...
    msLL.sex <- function(pv, pn, scrFrame, D, Y, nG, nK, hiK, 
        dm.den, dm.trap) {
