# oSCR.fit with Kern caching optimisation
# 
# Change from original:
#   D[[s]]^theta (the distance matrix raised to power theta) was recomputed
#   inside the likelihood function on every optimizer iteration. Since D and
#   theta are fixed throughout optimisation, D^theta is now precomputed once
#   per session before nlm() is called and stored in 'Dtheta'. This is passed
#   as an argument to msLL.nosex and msLL.sex and used in place of D[[s]]^theta
#   in all kernel computations.
# 
# Usage:
#   library(oSCR)
#   source('oSCR.fit.kern_cache.R')  # overwrites oSCR.fit in the search path
#   # then call oSCR.fit() as normal

oSCR.fit <-
  function (model = list(D ~ 1, p0 ~ 1, sig ~ 1, asu ~ 1), scrFrame, 
            ssDF, encmod = c("B", "P", "CLOG", "M")[1], multicatch = FALSE, 
            theta = 2, trimS = NULL, DorN = c("D", "N")[1], sexmod = c("constant", 
                                                                       "session")[1], costDF = NULL, distmet = c("euc", "user", 
                                                                                                                 "ecol")[1], directions = 8, PROJ = NULL, rsfDF = NULL, 
            RSF = FALSE, telemetry.type = c("none", "ind", "dep")[1], 
            se = TRUE, predict = FALSE, start.vals = NULL, getStarts = FALSE, 
            pxArea = 1, plotit = F, mycex = 1, nlmgradtol = 1e-06, nlmstepmax = 10, 
            nlmiterlim = 200, smallslow = FALSE, print.level = 0) 
  {
    ptm <- proc.time()
    starttime <- format(Sys.time(), "%H:%M:%S %d %b %Y")
    my.model.matrix <- function(form, data) {
      cont.arg <- lapply(data.frame(data[, sapply(data.frame(data), 
                                                  is.factor)]), contrasts, contrasts = FALSE)
    ...
    if ((length(labels(terms(model[[4]]))) > 0) & distmet == 
        "euc") {
    ...
    if ("session" %in% all.vars(model[[2]]) & (!nnnn)) {
      for (s in 1:ns) {
        for (m in 1:length(scrFrame$trapCovs[[s]])) {
          sesf <- factor(rep(s, nrow(scrFrame$trapCovs[[s]][[m]])), 
                         levels = 1:ns)
    ...
    if (length(trap.fx) > 0) {
      ...
      mod2 <- update(model[[2]], ~. - sex - session - t - b - 
                       b:sex - sex:b - b:session - session:b - sex:session - 
                       session:sex - b:session:sex - b:sex:session - sex:session:b - 
      ...
      for (s in 1:ns) {
        tmp.dm <- list()
        for (k in 1:nK[s]) {
          tmp.dm[[k]] <- model.matrix(mod2, scrFrame$trapCovs[[s]][[k]])[, 
                                                                         -1, drop = FALSE]
          if (s == 1 && k == 1) 
        ...
        dm.trap[[s]] <- tmp.dm
        if (RSF) {
          dm.rsf[[s]] <- model.matrix(mod2, rsfDF[[s]])[, 
                                                        -1, drop = FALSE]
        }
      }
    ...
    if (DorN == "N") {
      if (length(dens.fx) > 0) {
        ...
        for (s in 1:ns) {
          dm.den[[s]] <- model.matrix(mod1, ssDF[[s]])[, 
                                                       -1, drop = FALSE]
          if (s == 1) 
    ...
    if (distmet == "ecol") {
      ...
      pars.c0 <- 0.01
      pars.dist <- c(pars.c0, rep(0.01, length(names.dist) - 
                                    1))
    }
    if (!smallslow) {
      if (distmet == "euc") {
    ...
    Dtheta <- lapply(seq_len(ns), function(s) Dtheta[[s]])
    ...
    if (!var.p0.1 & var.p0.2) {
      if (ns > 1) {
        tmp.p0.names <- c("p0.(Intercept)", paste0("p0.session", 
                                                   2:ns))
        pJustsesh <- TRUE
      }
      else {
    ...
    if (var.p0.4) {
      if (ns > 1) {
        tmp.p0.names <- c("p0.(Intercept)", paste0("p0.f.session", 
                                                   2:ns), paste0("p0.m.session", 1:ns))
        pBothsexnsesh <- TRUE
      }
      else {
        tmp.p0.names <- c("p0.(Intercept)", "p0.male")
    ...
    if (encmod %in% c("B", "CLOG")) {
      st.p0 <- qlogis(0.1 * (sum(sapply(scrFrame$caphist, sum))/sum(sapply(scrFrame$caphist, 
    ...
    msLL.nosex <- function(pv = pv, pn = pn, D = D, Dtheta = Dtheta, hiK = hiK, 
                           nG = nG, nK = nK, dm.den = dm.den, dm.trap = dm.trap) {
      alpha0 <- array(0, dim = c(ns, hiK, 2))
      tmpP <- pv[pn %in% names.p0[grep(fixed = TRUE, "p0.(Intercept)", 
                                       names.p0)]]
      if (pDot & !pTime) {
        alpha0[, , ] <- tmpP
      }
      if (pDot & pTime) {
        tmpT <- c(0, pv[pn %in% names.p0[grep(fixed = TRUE, 
                                              "p0.t", names.p0)]])
        for (s in 1:ns) {
      ...
      if (pJustsesh & !pTime) {
        tmpSS <- c(0, pv[pn %in% names.p0[grep(fixed = TRUE, 
                                               "p0.session", names.p0)]])
        for (s in 1:ns) {
      ...
      if (bDot) {
        BRmat[, , 1] <- pv[pn %in% names.p0[grep(fixed = TRUE, 
                                                 "p.behav", names.p0)]]
      }
      if (bJustsesh) {
        for (k in 1:hiK) {
          BRmat[, k, 1] <- pv[pn %in% names.p0[grep(fixed = TRUE, 
                                                    "p.behav.session", names.p0)]]
        }
      }
      alpha0[, , 2] <- alpha0[, , 1] + BRmat[, , 1]
      sig.beta <- pv[grep("sig.", pn)]
      alphsig <- model.matrix(model[[3]], scrFrame$sigCovs) %*% 
        sig.beta
      ...
      for (s in 1:length(scrFrame$caphist)) {
        ...
        if (telem) {
          Ytels <- YYtel[[s]]
          cap.tel <- scrFrame$telemetry$cap.tel[[s]]
        ...
        if (telemetry.type == "dep") {
          lik.cond.tel <- matrix(0, nrow = length(cap.tel), 
                                 ncol = nG[s])
        }
        Kern <- exp(-alphsig[s] * Dtheta[[s]])
        for (i in 1:nrow(Ys)) {
          lik.cond <- numeric(nG[s])
          for (k in 1:nK[s]) {
            if (plotit) {
              pp <- sum(trimR[[s]][[i]][[k]])
              plot(ssDF[[s]][, c("X", "Y")], pch = 16, 
                   col = "grey", cex = 0.5, asp = 1, main = paste("Session:", 
                                                                  s, " Individual: ", i, " traps: ", sum(pp), 
                                                                  sep = " "))
              points(ssDF[[s]][trimC[[s]][[i]], c("X", 
                                                  "Y")], pch = 16, col = 2, cex = mycex)
              points(scrFrame$traps[[s]][, c("X", "Y")], 
                     pch = 15, col = "grey", cex = 1)
              points(scrFrame$traps[[s]][trimR[[s]][[i]][[k]], 
                                         c("X")], scrFrame$traps[[s]][trimR[[s]][[i]][[k]], 
                                                                      c("Y")], pch = 15, col = 1, cex = 1)
              pp <- apply(Ys[i, , ], 1, sum) > 0
              points(scrFrame$traps[[s]][pp, c("X", "Y")], 
                     pch = 15, col = 3, cex = mycex)
            }
            if (!("removed" %in% names(scrFrame$indCovs[[s]]))) {
              ...
              if (i < nrow(Ys)) {
                dead <- ifelse(k > scrFrame$indCovs[[s]]$removed[i], 
                               0, 1)
              }
              else {
                dead <- 1
            ...
            if (off.p$offset) {
              offset.location <- which(colnames(scrFrame$trapCovs[[s]][[k]]) %in% 
                                         off.p$name)
              offset.col <- scrFrame$trapCovs[[s]][[k]][, 
                                                        offset.location]
              a0 <- a0 + offset.col
            }
            if (encmod == "B") 
            ...
            if (!multicatch) {
              ...
              if (encmod == "CLOG") {
                probcap[1:length(probcap)] <- c(dbinom(rep(Ys[i, 
                                                              trimR[[s]][[i]][[k]], k], sum(trimC[[s]][[i]])), 
                                                       1, 1 - exp(-probcap[1:length(probcap)]), 
                                                       log = TRUE))
              }
            }
            else {
              ...
              vvv <- rep(c(Ys[i, trimR[[s]][[i]][[k]], 
                              k], 1 - any(Ys[i, trimR[[s]][[i]][[k]], 
                                             k] > 0)), sum(trimC[[s]][[i]]))
            ...
            if (is.matrix(probcap)) 
              lik.cond[trimC[[s]][[i]]] <- lik.cond[trimC[[s]][[i]]] + 
                colSums(probcap)
            if (!is.matrix(probcap)) 
              lik.cond[trimC[[s]][[i]]] <- lik.cond[trimC[[s]][[i]]] + 
                probcap
        ...
        if (telem) {
          if (RSF) {
            ...
            rsf.lam0 <- array(rsf.lam0, dim = c(nrow(rsfDF[[s]]), 
                                                nrow(ssDF[[s]])))
          }
          else {
            rsf.lam0 <- 0
          ...
          for (i in 1:nrow(Ytels)) {
            ...
            lik.marg.tel[i] <- sum(exp(Ytels[i, , drop = F] %*% 
                                         log.probs) * as.vector(pi.s))
            if (telemetry.type == "dep") {
              if (i <= length(cap.tel)) {
                lik.cond.tot <- (Ytels[i, , drop = F] %*% 
                                   log.probs) + lik.cond.tel[i, ]
                lik.cond.tot[is.na(lik.cond.tot)] <- 0
                lik.cond.tot[lik.cond.tot != 0] <- exp(lik.cond.tot[lik.cond.tot != 
                                                                      0])
                lik.marg[cap.tel[i]] <- sum(lik.cond.tot * 
                                              as.vector(pi.s))
                lik.marg.tel[i] <- 1
                if (predict) {
                  preds[[s]][cap.tel[i], ] <- lik.cond.tot * 
                    as.vector(pi.s)/lik.marg[cap.tel[i]]
                }
              }
            }
          }
    ...
    msLL.sex <- function(pv, pn, scrFrame, D, Dtheta, Y, nG, nK, hiK, 
                         dm.den, dm.trap) {
      ...
      if (pJustsex & !pTime & !pJustsesh & !pBothsexnsesh) {
        tmpS <- pv[pn %in% names.p0[grep(fixed = TRUE, "p0.male", 
                                         names.p0)]]
        for (s in 1:ns) {
          alpha0[s, , 1, 1] <- tmpP
      ...
      if (pBothsexnsesh & !pTime) {
        tmpSSF <- c(0, pv[pn %in% names.p0[grep(fixed = TRUE, 
                                                "p0.f.session", names.p0)]])
        tmpSSM <- pv[pn %in% names.p0[grep(fixed = TRUE, 
                                           "p0.m.session", names.p0)]]
        for (k in 1:hiK) {
      ...
      if (bJustsex) {
        BRmat[, , 1, 1] <- pv[pn %in% names.p0[grep(fixed = TRUE, 
                                                    "p.behav.f", names.p0)]]
        BRmat[, , 2, 1] <- pv[pn %in% names.p0[grep(fixed = TRUE, 
                                                    "p.behav.m", names.p0)]]
      }
      if (bJustsesh) {
        for (k in 1:hiK) {
          BRmat[, k, 1, 1] <- pv[pn %in% names.p0[grep(fixed = TRUE, 
                                                       "p.behav.session", names.p0)]]
          BRmat[, k, 2, 1] <- pv[pn %in% names.p0[grep(fixed = TRUE, 
                                                       "p.behav.session", names.p0)]]
        }
      }
      if (bBothsexnsesh) {
        for (k in 1:hiK) {
          BRmat[, k, 1, 1] <- pv[pn %in% names.p0[grep(fixed = TRUE, 
                                                       "p.behav.f.session", names.p0)]]
          BRmat[, k, 2, 1] <- pv[pn %in% names.p0[grep(fixed = TRUE, 
                                                       "p.behav.m.session", names.p0)]]
        }
      }
      ...
      if (!n0Session) 
        n0 <- rep(exp(pv[pn %in% names.n0]), ns)
      if (sexmod == "constant") 
        psi.sex <- rep(plogis(pv[grep(fixed = TRUE, "psi", 
                                      pn)]), ns)
      if (sexmod == "session") 
      ...
      for (s in 1:length(scrFrame$caphist)) {
        ...
        if (telem) {
          Ytels <- YYtel[[s]]
          sxtel <- scrFrame$telemetry$indCovs[[s]]$sex + 
            1
          cap.tel <- scrFrame$telemetry$cap.tel[[s]]
          lik.marg.tel <- rep(NA, nrow(Ytels))
        }
        ...
        for (i in 1:nrow(Ys)) {
          ...
          if (!is.na(sx[i])) {
            ...
            for (k in 1:nK[s]) {
              ...
              if (is.matrix(probcap)) 
                lik.cond1[trimC[[s]][[i]]] <- lik.cond1[trimC[[s]][[i]]] + 
                  colSums(probcap)
              if (!is.matrix(probcap)) 
                lik.cond1[trimC[[s]][[i]]] <- lik.cond1[trimC[[s]][[i]]] + 
                  probcap
              ...
              if (is.matrix(probcap)) 
                lik.cond2[trimC[[s]][[i]]] <- lik.cond2[trimC[[s]][[i]]] + 
                  colSums(probcap)
              if (!is.matrix(probcap)) 
                lik.cond2[trimC[[s]][[i]]] <- lik.cond2[trimC[[s]][[i]]] + 
                  probcap
        ...
        if (telem) {
          ...
          tol <- 1e-25
          for (i in 1:nrow(Ytels)) {
            probs <- t(exp(rsf.lam0 - alphsig[s, sxtel[i]] * 
                             Drsf[[s]]^theta))
            denom <- rowSums(probs)
            probs <- t(probs/denom)
            log.probs <- log(probs)
            ...
            if (telemetry.type == "dep") {
              if (i <= length(cap.tel)) {
                ...
                lik.cond.tot[lik.cond.tot != 0] <- exp(lik.cond.tot[lik.cond.tot != 
                                                                      0])
                tmpPsi <- (sx[cap.tel[i]] == 1) * (1 - 
                                                     psi.sex[s]) + (sx[cap.tel[i]] == 2) * 
                  psi.sex[s]
                lik.marg[cap.tel[i]] <- sum(lik.cond.tot * 
                                              as.vector(pi.s)) * tmpPsi
                lik.marg.tel[i] <- 1
                if (predict) {
                  preds[[s]][cap.tel[i], ] <- lik.cond.tot * 
                    as.vector(pi.s) * tmpPsi/lik.marg[cap.tel[i]]
                }
              }
            }
          }
      ...
      if (!predict) {
        ...
        if (!anySex) {
          ...
          myfit <- suppressWarnings(nlm(msLL.nosex, p = pv, 
                                        pn = pn, D = D, nG = nG, nK = nK, hiK = hiK, 
                                        dm.den = dm.den, dm.trap = dm.trap, hessian = hessian, 
                                        print.level = print.level, iterlim = nlmiterlim))
        }
        else {
          if (telem) {
            message("Telemetry: ", telemetry.type)
          ...
          message(paste(pn, " ", sep = " | "))
          message(" ")
          myfit <- suppressWarnings(nlm(msLL.sex, scrFrame = scrFrame, 
                                        p = pv, pn = pn, D = D, nG = nG, nK = nK, hiK = hiK, 
                                        dm.den = dm.den, dm.trap = dm.trap, hessian = hessian, 
                                        print.level = print.level, iterlim = nlmiterlim))
        }
        ...
        if (encmod == "B") {
          trans.mle[grep(fixed = TRUE, "p0.(Intercept)", 
                         pn)] <- plogis(pars[grep(fixed = TRUE, "p0.(Intercept)", 
                                                  pn)])
        }
        else {
          trans.mle[grep(fixed = TRUE, "p0.(Intercept)", 
                         pn)] <- exp(pars[grep(fixed = TRUE, "p0.(Intercept)", 
                                               pn)])
        }
        trans.mle[grep(fixed = TRUE, "sig.(Intercept)", pn)] <- exp(pars[grep(fixed = TRUE, 
                                                                              "sig.(Intercept)", pn)])
        trans.mle[grep(fixed = TRUE, "n0.", pn)] <- exp(pars[grep(fixed = TRUE, 
                                                                  "n0.", pn)])
        trans.mle[grep(fixed = TRUE, "d0.(Intercept)", pn)] <- exp(pars[grep(fixed = TRUE, 
                                                                             "d0.(Intercept)", pn)])
        trans.mle[grep(fixed = TRUE, "c0.(Intercept)", pn)] <- exp(pars[grep(fixed = TRUE, 
                                                                             "c0.(Intercept)", pn)])
        trans.mle[grep(fixed = TRUE, "psi", pn)] <- plogis(pars[grep(fixed = TRUE, 
                                                                     "psi", pn)])
        trans.mle[grep(fixed = TRUE, "beta", pn)] <- pars[grep(fixed = TRUE, 
                                                               "beta", pn)]
        if (pBehave) {
          links[grep(fixed = TRUE, "pBehav", pn)] <- "(Identity)"
          trans.mle[grep(fixed = TRUE, "pBehav", pn)] <- pars[grep(fixed = TRUE, 
                                                                   "pBehav", pn)]
        }
        ...
        outStats <- data.frame(parameters = pn, mle = myfit$estimate, 
                               std.er = std.err)
        VcV <- NULL
        if (DorN == "N") {
        }
        ...
        output <- list(call = cl, model = model.call, rawOutput = myfit, 
                       outStats = outStats, coef.mle = data.frame(param = pn, 
                                                                  mle = myfit$estimate), Area = areaS, nObs = unlist(lapply(scrFrame$caphist, 
                       ...
                       started = starttime, ended = endtime, proctime = (proc.time() - 
                                                                           ptm)[3]/60, scrFrame = scrFrame, ssDF = ssDF, 
                       costDF = costDF, rsfDF = rsfDF)
      ...
      if (predict) {
        ...
        if (!anySex) {
          ...
          myfit <- msLL.nosex(p = start.vals, pn = pn, 
                              D = D, hiK = hiK, nG = nG, nK = nK, dm.den = dm.den, 
                              dm.trap = dm.trap)
        }
        else {
          if (telem) {
          ...
          message(paste(pn, " ", sep = " | "))
          message(" ")
          myfit <- msLL.sex(scrFrame = scrFrame, p = start.vals, 
                            pn = pn, D = D, nK = nK, nG = nG, hiK = hiK, 
