oSCR.fit.cs <-function (model = list(D ~ 1, p0 ~ 1, sig ~ 1, asu ~ 1), scrFrame, ssDF = NULL, 
                        costDF = NULL, distmet = c("euc", "user","ecol")[1], encmod = c("B","P")[1], 
                        sexmod = c("constant", "session")[1], trimS = NULL, getStarts = FALSE, 
                        DorN = c("D", "N")[1], directions = 8, Dmat = NULL, start.vals = NULL, 
                        PROJ = NULL, pxArea = 1, plotit = F, mycex = 0.5, tester = F, 
                        predict = FALSE, smallslow = FALSE, multicatch = FALSE,
                        gr = NULL, method = "Nelder-Mead", lower = -Inf, upper = Inf,
                        control = list(trace = 0, maxit = 500), hessian = FALSE, trace = 0, maxit = 200){
  ...
  if(getStarts == FALSE){
    if (!predict) {
      ...
      if (!anySex) {
        ...
        message(" ")
        myfit <- suppressWarnings(optim(p = pv, msLL.nosex, 
        ...
        message(paste(pn, " ", sep = " | "))
        message(" ")
        myfit <- suppressWarnings(optim(p = pv, msLL.sex,
                                        pn = pn, YY = YY, D = D, nG = nG, nK = nK,
                                        hiK = hiK, dm.den = dm.den, dm.trap = dm.trap,
                                        gr = gr, method = method,
                                        lower = lower, upper = upper,
      ...
      outStats <- data.frame(parameters = pn, link = links,
                             mle = myfit$par, std.er = std.err, mle.tr = trans.mle,
                             se.tr = trans.se)
      VcV <- NULL
      if (DorN == "N") {
      ...
      output <- list(call = cl, rawOutput = myfit, outStats = outStats,
                     ...
                     Area = areaS, ED = ED, nObs = unlist(lapply(scrFrame$caphist,
                                                                 nrow)), mmdm = mmdm, nll = myfit$value, AIC = 2 *
                       myfit$value + 2 * length(myfit$par),
